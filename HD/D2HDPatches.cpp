...
void HD::Replace640_ResizeWindow_Interception() {
	__asm {
		mov dword ptr ds : [eax], RESOLUTION_HD_WIDTH; Requires registers due to the
		mov dword ptr ds : [ecx], RESOLUTION_HD_HEIGHT; use of ESP to access memory
	}
}

void __declspec(naked) HD::Replace640_ResizeRenderResolution_Interception() {
	__asm {
		mov esi, (RESOLUTION_HD_WIDTH - (RESOLUTION_HD_WIDTH % 4))
		mov edx, RESOLUTION_HD_HEIGHT
		ret
	}
}

void HD::Replace640_ResizeForgroundRenderWidth_Interception() {
	*D2GDI_ForegroundRenderWidth = RESOLUTION_HD_WIDTH;
}

int HD::Replace640_ResizeGameLogicResolution_Interception() {
	*D2CLIENT_ScreenSizeX = RESOLUTION_HD_WIDTH;
	*D2CLIENT_ScreenSizeY = RESOLUTION_HD_HEIGHT;
	return *D2CLIENT_ScreenSizeX;
}

void HD::Replace640_ResizeGlideRenderResolution_Interception() {
	*D2GLIDE_ScreenSizeX = RESOLUTION_HD_WIDTH;
	*D2GLIDE_ScreenSizeY = RESOLUTION_HD_HEIGHT;
}
...
void HD::Replace640_ResizeGlideWindow_Interception() {
	...
	__asm {
		mov edx, pWidth
		mov dword ptr ds : [edx], RESOLUTION_HD_WIDTH
		mov ecx, pHeight
		mov dword ptr ds : [ecx], RESOLUTION_HD_HEIGHT
	}
}
